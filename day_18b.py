...
for i in range(len(parsed_arr)):
    for j in range(len(parsed_arr)):
        if i == j:
            continue
        left_node_dict, left_start = make_tree(parsed_arr[i], 0)
        right_node_dict, right_start = make_tree(parsed_arr[j], max(left_node_dict.keys()) + 1)
        ...
        max_magnitude = max(max_magnitude, find_magnitude(left_start))
